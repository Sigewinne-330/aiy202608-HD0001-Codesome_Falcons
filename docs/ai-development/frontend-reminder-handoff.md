...
2. **角色卡选择器与详情弹窗**：展示三个内置角色卡，并允许用户选择其中之一；同一选择同时作用于提醒和主 AI 聊天。
...
- 提醒文案语言由 `language` 决定；角色卡只影响语气，不影响语言和提醒规则。主聊天仍按用户当前对话语言回复。
- `role_card_id` 是提醒 Agent 与主 Agent 的共享选择。保存成功后从下一条主聊天消息和后续提醒生效，已有聊天记录不会重写，也不需要新建对话。
...
| IBuddy 交流风格 | 卡片选择器 | `role_card_id` | 同时控制提醒与主聊天口吻；无选择时传 `null` 以恢复默认卡。|
...
该字段仅用于未来展示“当前交流风格”或调试来源。`status` 可能为 `selected`、`default`、`neutral` 或 `disabled`；前端不得依赖完整角色卡 prompt，因为后端不会返回或持久化它。
...
- [ ] 选择角色卡后无需新建会话，下一条主聊天与后续提醒使用同一口吻；旧消息不变化。
